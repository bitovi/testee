...
_.extend(Reporter.prototype, {
  ...
  setup: function(runs, suites, tests, hooks, coverages, logs) {
    ...
    hooks.on('created', this.proxy('startHook'));
  ...
  startHook: function(data) {
    debug('starting hook', data);
    this.buffers.updateCache(data);
    ...
    // Emit the event that a hook started
  ...
  updateHook: function(data) {
    ...
    // emit the events on the runner appropriate for the hook result, we only care about failed hooks
    switch(data.status) {
      case 'failed':
        // The second argument is the error which can be passed right through
