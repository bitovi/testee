...
var launch = function(url, config, callback) {
	...
	launchpad[type](launcherSettings, function(error, local) {
		if(error) return callback(error);

		local(url, browserSettings, callback);
	});
}
...
var test = function(file, configuration, callback) {
	...
	var server = Server.createServer(_.pick(config, 'root'));

	config.root = fs.realpathSync(config.root);

	// First thing: Create the tunnel
	createTunnel(_.pick(config, 'port', 'tunnel'), function(error, hostUrl, process) {
		if(error) return callback(error);
